...
def train_model_now():
    """
    Runs the full pipeline (run_pipeline.py's main()) in-process and clears
    Streamlit's caches so the freshly saved artifacts get picked up.

    This exists mainly for cloud deployments (e.g. Streamlit Community
    Cloud): the models/ and data/*clustered.csv files are generated by
    run_pipeline.py and may not have been committed to the GitHub repo, so
    the deployed app has nothing to load on first boot. Training in-app
    means the app is self-sufficient even if only the source files (src/,
    app.py, data/students_social_network.csv) made it into the repo.
    """
    from run_pipeline import main as run_pipeline_main
    with st.spinner("No trained model found — training now "
                     "(K-Means, Hierarchical, DBSCAN + comparison, ~1-2 min)..."):
        run_pipeline_main()
    get_trained_artifacts.clear()
    get_clustered_data.clear()
    st.rerun()
...
st.sidebar.markdown("---")
model_exists = os.path.exists(config.KMEANS_MODEL_PATH)
if model_exists:
...
st.sidebar.caption("Or run `python run_pipeline.py` locally / at deploy time "
                    "to pre-generate the model artifacts.")
...
# ---------------------------------------------------------------------------
    ...
    if df_clustered is None:
        st.warning("No saved clustering found yet.")
        if st.button("🚀 Train model now", key="train_btn_profiles"):
            train_model_now()
    else:
...
# ---------------------------------------------------------------------------
    ...
    if artifacts["kmeans"] is None:
        ...
        if st.button("🚀 Train model now", key="train_btn_predict"):
            train_model_now()
    else:
